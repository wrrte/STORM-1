...
`convert_csv_to_excel.py`를 실행하면 현재 `STORM/job_queue.txt`와
`STORM/job_queue_*.txt`도 읽어 엑셀의 해당 게임·설정·시드 셀에 **QUEUED (파란색)**를
표시합니다. 스케줄러 등록 이력과 관계없이 직접 넣은 작업도 포함하며, `Both`와 실험
목록은 각 설정 행으로 펼칩니다. YAML·명령행 설정을 반영하고 `--resume_warmup`은
저장된 설정을 읽습니다. 다른 GPU에만 있어 warmup 설정을 읽을 수 없으면 해당
체크포인트가 정상 존재한다고 가정하고 `config_files/STORM.yaml` 기본값과 명령행
옵션으로 모든 실험(baseline, retrieval, target1, value, add)을 표시합니다. 큐 경로에
기본 설정 파일이 없으면 이 저장소의 기본 설정을 사용합니다. 게임·시드는
`ckpt/게임-시드`, `ckpt/게임-시드_Shared` 및 그 하위 checkpoint 경로에서 읽으며,
스케줄러도 이 경로들을 인식해 예약·GPU 예상 작업량에 반영합니다.
해석할 수 없는 명령은 큐 파일명·줄 번호와 함께 경고합니다.
기존 점수 뒤에 대기 표시를 붙이므로 평균·LaTeX 집계는 바뀌지 않습니다. 셀 메모에는
큐 파일·줄 번호·원래 명령이 들어갑니다. 실행 중인 셀은 노란색을 우선하며 제외 시드의
취소선과 warmup 저장의 초록색 테두리도 유지합니다. 큐에서 빠진 작업은 다음 변환 때
대기 표시가 사라집니다. 스케줄러로 큐를 변경한 뒤 최신 엑셀을 보려면 변환기를 다시
실행하세요. 변환기는 큐나 스케줄러 상태를 수정하지 않으며 어느 디렉터리에서 실행해도
기본 CSV·큐를 읽어 `STORM/results/converted_results.xlsx`를 생성합니다.
...
대기 예약은 매번 **현재 `job_queue_*.txt`와 자동으로 동기화**합니다. 큐에 없고 CSV에도
해당 작업의 실행/결과 기록이 없는 대기 작업은 상태 파일에서 자동 제거합니다.
등록 전에 이미 존재하던 과거 run은 이번 작업의 실행 증거로 사용하지 않습니다.
기존 버전에서 남은 `awaiting_result`도 같은 기준으로 정리합니다. 제거한 예약은 GPU
작업량과 사용한 시드 계산에서도 빠지며, 현재 `EXCLUDED_SEEDS`와 점수로 다시 계획합니다.

따라서 `EXCLUDED_SEEDS`를 수정하고 실행 전 명령을 큐에서 지운 뒤에는 평소처럼 실행하면
됩니다. 작업 ID나 별도의 정리 옵션을 입력할 필요가 없습니다.
...
큐에 남은 작업과 완료·실패 이력은 유지합니다. 실행을 확인한 작업도 계속 보관하므로
이후 CSV에서 누락되더라도 `awaiting_result`로 결과를 기다립니다. 실행하지 않은 시드는
새 계획에서 다시 선택될 수 있습니다. 큐에서 지웠다는 이유만으로 해당 시드를 영구
제외하지는 않습니다. `--dry-run`은 자동 정리도 미리보기만 합니다.

기존 worker는 실행할 명령을 큐에서 먼저 꺼내며 별도의 실행 상태 파일을 남기지 않습니다.
스케줄러는 CSV를 읽으므로 **worker가 가져갔지만 아직 CSV에 한 번도 반영되지 않은 작업**은
직접 삭제한 작업과 구별할 수 없습니다. worker가 동작 중이면 `classify_wandb_runs.py`로
실행 상태를 갱신한 뒤 스케줄러를 실행하세요.

실제로 시작했다가 종료·실패한 작업의 결과가 유실된 경우에는 다음처럼 실패로 기록합니다.
...
`job_queue_*.lock`을 잡은 동안 큐와 예약을 대조하고 명령을 추가합니다. 상태 저장 후
큐 쓰기 도중 장애가 나면, 다음 실행에서 큐와 실행 기록을 기준으로 누락된 예약을 정리합니다.
