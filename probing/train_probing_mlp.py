...
잠수부 개수(0~6) 또는 이글루 단계(0~16)를 classification하는 MLP를 학습합니다.
--noise_strength로 배경 노이즈 강도(0~1)를 지정합니다. 각 실행에서는 해당
강도의 데이터로 MLP를 새로 학습하고, 같은 강도의 데이터로 검증/평가합니다.
...
Usage:
    ...
    python train_probing_mlp.py \
        ...
        --task diver_count --noise_strength 0.5
"""
...
def inject_background_noise(obs_array, task, strength=1.0, seed=None):
    """
    타겟 영역을 보존하고 배경을 (1-strength)*원본 + strength*U(0..255)로 혼합.
    obs_array: (N, 64, 64, 3) uint8 numpy array
    strength: 0이면 원본, 1이면 기존의 완전한 배경 노이즈 교체.
    seed: 별도의 RNG를 사용하여 노이즈가 train/val 분할에 영향을 주지 않도록 함.
          같은 seed와 데이터는 강도에 관계없이 같은 랜덤 픽셀을 사용함.
    """
    if not 0.0 <= strength <= 1.0:
        raise ValueError("noise strength must be between 0 and 1")
    if strength == 0.0:
        return obs_array.copy()

    if task == "frostbite_igloo":
        target_y, target_x = slice(10, 17), slice(42, 58)
        target_name = "이글루 영역(X:42~58, Y:10~17)"
    else:
        target_y, target_x = slice(53, 57), slice(18, 46)
        target_name = "하단 잠수부 영역(X:18~46, Y:53~57)"
    print(f"  [Noise Injection] {target_name} 보존, 배경 노이즈 강도={strength:g}")

    rng = np.random.RandomState(seed)
    noisy_obs = np.empty_like(obs_array)
    # 큰 Seaquest 데이터도 전체 float32 사본 없이 처리.
    for start in range(0, len(obs_array), 1024):
        clean = obs_array[start:start + 1024]
        noise = rng.randint(0, 256, size=clean.shape, dtype=np.uint8)
        if strength == 1.0:
            mixed = noise
        else:
            mixed = np.rint(
                (1.0 - strength) * clean.astype(np.float32)
                + strength * noise.astype(np.float32)
            ).clip(0, 255).astype(np.uint8)
        mixed[:, target_y, target_x, :] = clean[:, target_y, target_x, :]
        noisy_obs[start:start + len(clean)] = mixed
    return noisy_obs
...
def main():
    ...
    noise_options.add_argument("--noise_bg", action="store_true",
                               help="배경을 균일 랜덤 노이즈로 완전히 교체 (--noise_strength 1과 동일)")
    noise_options.add_argument("--noise_strength", type=float, default=None,
                               help="배경 노이즈 혼합 강도 [0, 1]: 0=원본, 1=완전 교체. 학습/평가 모두 적용")
    args = parser.parse_args()
    noise_strength = args.noise_strength
    if noise_strength is None:
        noise_strength = 1.0 if args.noise_bg else 0.0
    if not 0.0 <= noise_strength <= 1.0:
        parser.error("--noise_strength must be between 0 and 1")
    ...
    print(f"  Background noise strength: {noise_strength:g} (train/validation/evaluation)")
    print(f"{'='*60}\n")
    ...
    if noise_strength > 0.0:
        obs_array = inject_background_noise(obs_array, args.task, strength=noise_strength, seed=args.seed)
    ...
    if args.noise_strength is not None:
        result_name += f"_noise_strength_{noise_strength}"
    elif args.noise_bg:
    ...
    results = {
        ...
        "noise_method": "background_blend_uniform_uint8",
        "noise_applied_to": "train_validation_and_full_dataset_evaluation",
